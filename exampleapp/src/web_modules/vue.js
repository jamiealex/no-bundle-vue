...
function trigger$1$1(target, type, key, newValue, oldValue, oldTarget) {
    ...
    if (type === "clear" /* CLEAR */) {
        ...
        if (isAddOrDelete ||
            (type === "set" /* SET */ && target instanceof Map)) {
            add(depsMap.get(isArray(target) ? 'length' : ITERATE_KEY$1));
        }
        if (isAddOrDelete && target instanceof Map) {
...
function createSetter$1(shallow = false) {
    return function set(target, key, value, receiver) {
        ...
        if (target === toRaw$1(receiver)) {
            if (!hadKey) {
                trigger$1$1(target, "add" /* ADD */, key, value);
            }
            else if (hasChanged(value, oldValue)) {
                trigger$1$1(target, "set" /* SET */, key, value, oldValue);
            }
        }
        return result;
    };
...
function deleteProperty$1(target, key) {
    ...
    const result = Reflect.deleteProperty(target, key);
    if (result && hadKey) {
        trigger$1$1(target, "delete" /* DELETE */, key, undefined, oldValue);
    }
    return result;
}
...
function get$1$1(target, key, wrap) {
...
function has$1$1(key) {
    ...
    const rawKey = toRaw$1(key);
    const has = getProto$1(target).has;
    return has.call(target, key) || has.call(target, rawKey);
}
function size$1(target) {
    target = toRaw$1(target);
    return Reflect.get(getProto$1(target), 'size', target);
}
function add$1(value) {
    ...
    const result = proto.add.call(target, value);
    if (!hadKey) {
        trigger$1$1(target, "add" /* ADD */, value, value);
    }
    return result;
}
function set$1$1(key, value) {
    value = toRaw$1(value);
    const target = toRaw$1(this);
    const { has, get, set } = getProto$1(target);
    let hadKey = has.call(target, key);
    if (!hadKey) {
        key = toRaw$1(key);
        hadKey = has.call(target, key);
...
function deleteEntry$1(key) {
    const target = toRaw$1(this);
    const { has, get, delete: del } = getProto$1(target);
    let hadKey = has.call(target, key);
    if (!hadKey) {
        key = toRaw$1(key);
        hadKey = has.call(target, key);
...
function clear$1() {
    ...
    const oldTarget =  target instanceof Map
        ...
        ;
    // forward the operation before queueing reactions
    const result = getProto$1(target).clear.call(target);
    if (hadItems) {
        trigger$1$1(target, "clear" /* CLEAR */, undefined, undefined, oldTarget);
    }
    return result;
}
function createForEach$1(isReadonly) {
    return function forEach(callback, thisArg) {
        const observed = this;
        const target = toRaw$1(observed);
        const wrap = isReadonly ? toReadonly$1 : toReactive$1;
        // important: create sure the callback is
        // 1. invoked with the reactive map as `this` and 3rd arg
        // 2. the value received should be a corresponding reactive/readonly.
        function wrappedCallback(value, key) {
        ...
        return getProto$1(target).forEach.call(target, wrappedCallback, thisArg);
    };
}
function createIterableMethod$1(method, isReadonly) {
    return function (...args) {
        const target = toRaw$1(this);
        const isMap = target instanceof Map;
        const isPair = method === 'entries' || (method === Symbol.iterator && isMap);
        const innerIterator = getProto$1(target)[method].apply(target, args);
        const wrap = isReadonly ? toReadonly$1 : toReactive$1;
        // return a wrapped iterator which returns observed versions of the
        // values emitted from the real iterator
        return {
...
const mutableInstrumentations$1 = {
    get(key) {
        return get$1$1(this, key, toReactive$1);
    },
    get size() {
        return size$1(this);
    },
    ...
    clear: clear$1,
    forEach: createForEach$1(false)
};
const readonlyInstrumentations$1 = {
    get(key) {
        return get$1$1(this, key, toReadonly$1);
    },
    get size() {
        return size$1(this);
    },
    ...
    forEach: createForEach$1(true)
};
const iteratorMethods$1 = ['keys', 'values', 'entries', Symbol.iterator];
iteratorMethods$1.forEach(method => {
    mutableInstrumentations$1[method] = createIterableMethod$1(method, false);
    readonlyInstrumentations$1[method] = createIterableMethod$1(method, true);
});
function createInstrumentationGetter$1(instrumentations) {
    return (target, key, receiver) => Reflect.get(hasOwn(instrumentations, key) && key in target
        ? instrumentations
        : target, key, receiver);
}
const mutableCollectionHandlers$1 = {
    get: createInstrumentationGetter$1(mutableInstrumentations$1)
};
const readonlyCollectionHandlers$1 = {
    get: createInstrumentationGetter$1(readonlyInstrumentations$1)
};
...
// WeakMaps that store {raw <-> observed} pairs.
const rawToReactive$1 = new WeakMap();
const reactiveToRaw$1 = new WeakMap();
const rawToReadonly$1 = new WeakMap();
const readonlyToRaw$1 = new WeakMap();
// WeakSets for values that are marked readonly or non-reactive during
// observable creation.
const rawValues$1 = new WeakSet();
const collectionTypes$1 = new Set([Set, Map, WeakMap, WeakSet]);
const isObservableType$1 = /*#__PURE__*/ makeMap('Object,Array,Map,Set,WeakMap,WeakSet');
const canObserve$1 = (value) => {
    return (!value._isVue &&
        !value._isVNode &&
        isObservableType$1(toRawType(value)) &&
        !rawValues$1.has(value) &&
        !Object.isFrozen(value));
};
function reactive$1(target) {
    // if trying to observe a readonly proxy, return the readonly version.
    if (readonlyToRaw$1.has(target)) {
        return target;
    }
    return createReactiveObject$1(target, rawToReactive$1, reactiveToRaw$1, mutableHandlers$1, mutableCollectionHandlers$1);
}
function readonly$1(target) {
    return createReactiveObject$1(target, rawToReadonly$1, readonlyToRaw$1, readonlyHandlers$1, readonlyCollectionHandlers$1);
}
function createReactiveObject$1(target, toProxy, toRaw, baseHandlers, collectionHandlers) {
    if (!isObject(target)) {
        {
            console.warn(`value cannot be made reactive: ${String(target)}`);
        }
    ...
    // only a whitelist of value types can be observed.
    if (!canObserve$1(target)) {
        return target;
    }
    const handlers = collectionTypes$1.has(target.constructor)
        ? collectionHandlers
        : baseHandlers;
    observed = new Proxy(target, handlers);
    toProxy.set(target, observed);
...
function toRaw$1(observed) {
    observed = readonlyToRaw$1.get(observed) || observed;
    return reactiveToRaw$1.get(observed) || observed;
}
